...
> **장중 실행 가드** — `--refresh`는 종가 확정 시각(16:00) 전에 돌리면 당일을 수집
> 대상에서 자동으로 뺀다. 장중에는 KRX가 당일 행을 '현재가'로 내려주는데
> `update_cache`가 증분이라 한 번 들어간 날짜는 재수집하지 않아, 장중가가 확정 종가
> 자리에 영구히 고정되기 때문이다. 의도적으로 장중 데이터를 넣으려면 `--force-today`.
